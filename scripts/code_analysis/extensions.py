...
def registry(config: dict) -> list[dict]:
    ...
    for row in rows:
        ...
        if not re.fullmatch(r'ext-[a-z0-9][a-z0-9-]{0,45}', channel) or channel in ids:
        ...
        families.add(family)
        if row.get('class') not in CLASSES or row.get('mode') not in {'source', 'vendor'}:
            raise ValueError('Extension requires a supported class and source/vendor mode')
        if not safe_file(row.get('adapter', '')) or not row['adapter'].startswith('scripts/code_analysis/adapters/') or not row['adapter'].endswith('.py'):
        ...
        for env, secret in row.get('secrets', {}).items():
            if not re.fullmatch(r'[A-Z][A-Z0-9_]*', env) or not re.fullmatch(r'[A-Z][A-Z0-9_]*', secret):
        ...
        if row.get('evidence_source', 'DETERMINISTIC') not in {'DETERMINISTIC', 'AI_ADVISORY'}:
...
def ingest(config: dict, artifacts: Path, identity: dict, run_id: str) -> tuple[list, dict]:
    ...
    for row in registry(config):
        ...
        try:
            ...
            if envelope.get('status') not in STATUSES or not isinstance(envelope.get('findings'), list):
            ...
            for finding in envelope['findings']:
                ...
                if (finding.get('severity') not in {'CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'INFO', 'UNKNOWN'}
                        or not isinstance(finding.get('message'), str) or not finding.get('rule_id')
