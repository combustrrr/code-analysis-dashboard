...
type Props = { onSubmitted: (request: { kind: string; ref: string }) => void; auth: Auth; open: boolean; close: () => void; repository?: string; host?: string; workflowBranch?: string; targets: Target[]; initial?: Target; follow: (target: string) => void };

export function AnalysisLauncher({ onSubmitted, auth, open, close, repository, host, workflowBranch, targets, initial, follow }: Props) {
  ...
  return <Modal title="Run source analysis" open={open} onCancel={close} footer={null} width={660}>
    <div className="analysis-launcher">
      ...
      {kind !== 'commit' && <Select aria-label="Available analysis targets" showSearch optionFilterProp="label" placeholder={kind === 'branch' ? 'Choose a branch' : 'Choose an open PR'} value={selected?.id} options={choices.filter(t => t.kind === kind).map(t => ({ value: t.id, label: t.label }))} onChange={reset}/>}
      ...
      {auth.available ? <Space wrap>
        {auth.session ? <><span>Signed in as {auth.session.login}</span><Button onClick={auth.signOut}>Sign out</Button><Button aria-label="Start analysis" type="primary" loading={busy} disabled={!valid || !liveTargets || !!submitted} onClick={launch}>Start analysis</Button></> : <Button type="primary" onClick={auth.signIn}>Sign in with GitHub to start analysis</Button>}
      </Space> : <Alert type="info" title="Direct launching needs administrator setup" description="Connect this instance to its authenticated launcher to start scans here. GitHub Actions remains available below."/>}
      ...
      <Space wrap><Button type="primary" disabled={!valid || !workflowBranch} href={valid && workflowBranch ? workflow : undefined} target="_blank" rel="noreferrer" onClick={() => { setHandoff(true); const current = targets.find(t => t.kind === kind && (kind === 'pr' ? t.pr === selected?.pr : t.branch === selected?.branch)); if (current) follow(current.id); }}>Open Run analysis in GitHub</Button>
        {handoff && <Button onClick={close}>Return to results</Button>}</Space>
      {handoff && <Alert type="info" showIcon title="Awaiting confirmation on GitHub" description="Opening GitHub does not submit a scan. After you confirm Run workflow, the request is queued for an available analysis slot. The dashboard shows the run once the next status publication arrives."/>}
