...
export function Repositories({auth,endpoint}:{endpoint?:string;auth:ReturnType<typeof useLaunchAuth>}) {
  ...
  return <section className="connections-panel" aria-label="Connect repository">
    ...
    {!auth.session?<Button onClick={auth.signIn} disabled={!auth.available}>Sign in with GitHub</Button>:<Space wrap><span>Signed in as {auth.session.login}</span><Button loading={busy} onClick={()=>void list()}>Load repositories</Button><Button disabled={busy} onClick={auth.signOut}>Sign out</Button></Space>}
