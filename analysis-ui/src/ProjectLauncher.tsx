...
export function ProjectLauncher({auth,repository,project,close}:{auth:Auth;repository:string;project:string;close:()=>void}) {
 ...
 const [request,setRequest]=useState<{run_id:number|null;request_id:string}>();const [status,setStatus]=useState<{phase:string;source_sha?:string;analyzed_sha?:string;completeness?:string;error?:string;producer?:{url:string;attempt:number;status:string}}>();
 ...
 const poll=async()=>{try{if(document.visibilityState==='visible'){const r=await auth.api<NonNullable<typeof status>>(`project-activity?repository=${encodeURIComponent(repository)}&project_id=${project}&request_id=${request.request_id}`);if(active){setStatus(r);setError('');}if(['published','failed','superseded'].includes(r.phase))return;}delay=15000;}catch(e){if(active)setError(String(e));delay=Math.min(delay*2,120000);}if(active)timer=window.setTimeout(poll,delay);};void poll();return()=>{active=false;window.clearTimeout(timer);};},[request?.request_id,auth.session,repository,project]);
