...
import {Alert,Button,Descriptions,Input,Modal,Radio,Select,Space,Steps} from 'antd';
...
export function ProjectLauncher({auth,repository,project,close}:{auth:Auth;repository:string;project:string;close:()=>void}) {
 ...
 return <Modal open title="Run analysis" onCancel={close} footer={null} width={760}>
  ...
  {!request&&<>
   ...
   {step===1&&<Alert type="info" showIcon title="Scanner readiness" description="The reviewed repository profile determines applicable scanners. Missing credentials or adapters remain explicit in the report. Branch heads are resolved again when this request is processed."/>}
